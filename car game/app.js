...
Opponents.style.left = Math.floor(Math.random() * 450) + "px";

}

//other car
for (i = 0; i < 3; i++) {
    let Opponents1 = document.createElement('div');
    Opponents1.setAttribute('class', 'Opponents1');
    Opponents1.y = ((i) * -350);
    Opponents1.style.top = Opponents1.y + "px";
    gameArea.appendChild(Opponents1);
    Opponents1.style.left = Math.floor(Math.random() * 450) + "px";
    
    }
//third other car
for (i = 0; i < 3; i++) {
    let Opponents2 = document.createElement('div');
    Opponents2.setAttribute('class', 'Opponents2');
    Opponents2.y = ((i) * -350);
    Opponents2.style.top = Opponents2.y + "px";
    gameArea.appendChild(Opponents2);
    Opponents2.style.left = Math.floor(Math.random() * 450) + "px";
...
//
function moveOpponents1(car) {
    let Opponents = document.querySelectorAll('.Opponents1');
    Opponents.forEach(function (item) {
    if (isCollide(car, item)) {
    endGame();
    }
    ...
    function moveOpponents2(car) {
        let Opponents = document.querySelectorAll('.Opponents2');
        Opponents.forEach(function (item) {
        if (isCollide(car, item)) {
        endGame();
        }
